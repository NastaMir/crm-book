...
Для регистрации **менеджера** или **оператора** также необходимо указать:
* внутренний номер телефона;
* ключ доступа к API http://hd.abcreg.ru.
 
Для получения ключа доступа к API необходимо зайти в свою учетную запись на http://hd.abcreg.ru. Нажать кнопку «Показать» под строчкой «Ключ доступа к API».
![](/assets/red-api.png)
